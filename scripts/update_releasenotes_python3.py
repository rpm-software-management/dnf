...
ISSUE_RE = re.compile(r'(?<!Related: )RhBug:(\d+)')

RELATED_RE = re.compile(r'Related: RhBug:(\d+)')

SIMILAR_RE = re.compile(r'\d{6,}')
...
class TestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.commit = self.repository.index.commit(
            'Version (RhBug:{})'.format(self.ISSUE.decode()))
    ...
    def _assert_iter_equal(self, actual, expected):
        ...
        self.assertTrue(all(
            ...
            itertools.zip_longest(actual, expected, fillvalue=object())))
