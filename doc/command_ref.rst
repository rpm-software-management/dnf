...
The updated packages could replace the old configuration files with the new ones
(the former are preserved with ``.rpmold`` suffix) or keep the older files
(the more up-to-date files will be installed with ``.rpmnew`` suffix).
This behavior is specified by each package itself following packaging guideline
and is not controlled package manager.
