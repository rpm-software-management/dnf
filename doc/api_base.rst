...
.. class:: dnf.Base
  ...
  .. method:: install(pkg_spec, reponame=None)
    ...
    Mark packages matching `pkg_spec` for installation. If `reponame` is not ``None``, only packages from the repository named `reponame` are taken into account. In that case, group and local file marking is not supported.
