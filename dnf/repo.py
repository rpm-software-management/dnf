...
def _download_metadata(repos):
    ...
    try:
        ...
        raise dnf.exceptions.DownloadError(e.args[1] or '<unspecified librepo error>')
    ...
    for target in targets:
        ...
        for error in target_errors:
            if error is None or error.startswith('Not finished') or error == 'Already downloaded':
                continue

            repo = target.cbdata
            repo._md_expire_cache()

            errors = repo_errors.setdefault(repo.id, [])
            errors.append(error)
