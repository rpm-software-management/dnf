...
class Base(object):
    ...
    def download_metadata(self, repos):
        errors = dnf.repo._download_metadata(repos)
        ...
        for repo in repos:
            if repo.metadata is None or not repo.metadata.fresh:
                ...
                if repo.id in errors:
                    error_message = "; ".join(errors[repo.id])

                logger.warning(_("Failed to synchronize cache for repo '{}'".format(repo.id)))
                logger.debug(_("Cannot download repomd.xml: {}\n".format(error_message)))
